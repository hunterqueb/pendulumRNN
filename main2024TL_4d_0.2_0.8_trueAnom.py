...
rLEO = np.array([(p/(1+e)),0])
vLEO = np.array([0,np.sqrt(muR*((2/rLEO[0])-(1/a)))])
TLEO = 2*np.pi*np.sqrt(a**3/muR)
...
t = genTimestep4EquiTrueAnom(numPoints,numPeriods,0,T)
...
def plotPredition(epoch,model,prediction='source',err=None,t=t * TU,output_seq = circNR):
        ...
        if prediction == 'source':
            plt.savefig('predict/predict%d.pdf' % epoch)
        if prediction == 'target':
            plt.savefig('predict/newPredict%d.pdf' % epoch)
        plt.close()
