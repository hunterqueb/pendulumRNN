...
def get_args():
    ...
    parser.add_argument(
        "--trainDim", type=int, choices=[3, 6], default=6,
        help="Training dimension: 3 or 6 (default: 6)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--seqLength", type=int, default=1000,
        help="Length of the desired training data sequence (default: 1000)"
    )
...
print(f"Seq. Length of Data   : {seqLength}")
...
for i in range(numRandSys):
    ...
    tODE_beforeBurn,yODE_beforeBurn = ode87(fun=lambda t, y: twoBodyJ2(t, y, mu), tspan=(t0, t1_beforeBurn), y0=y0, t_eval = t_beforeBurn,rtol=1e-8, atol=1e-10)
    ...
    tODE_Burn,yODE_Burn = ode87(fun=lambda t, y: twoBodyJ2(t, y, mu), tspan=(t1_beforeBurn, t2_Burn), y0=y_burn1, t_eval = t_Burn,rtol=1e-8, atol=1e-10)

    tOriginalCircOrbit,yOriginalCircOrbit = ode87(fun=lambda t, y: twoBodyJ2(t, y, mu),tspan=(0, t2_Burn),y0=y0,t_eval = np.linspace(0,t2_Burn,int(seqLength*2)), rtol=1e-8, atol=1e-10)
    tOriginalCircOrbitPlot,yOriginalCircOrbitPlot = ode87(fun=lambda t, y: twoBodyJ2(t, y, mu),tspan=(0, tf),y0=y0,t_eval = np.linspace(0,tf,int(seqLength*2)), rtol=1e-8, atol=1e-10)
    ...
    yODE = apply_noise(yODE, pos_noise_std, vel_noise_std)
    yOriginalCircOrbit = apply_noise(yOriginalCircOrbit, pos_noise_std, vel_noise_std)

    yODE = yODE[:,:trainDim]
    yOriginalCircOrbit = yOriginalCircOrbit[:,:trainDim]
    yOriginalCircOrbitPlot = yOriginalCircOrbitPlot[:,:trainDim]
