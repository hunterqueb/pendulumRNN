...
orbitalElements = (12871.5,0.3629,np.deg2rad(42.5),0,0,0)

T = 2*np.pi*np.sqrt(orbitalElements[0]**3/muR)

cartesianIC = OE2ECI(orbitalElements)

IC = np.array(cartesianIC)
print(IC)
device = getDevice()
...
t,numericResult = ode45(system,[t0,tf],IC)

# plot 3d orbit
fig = plt.figure(figsize=(10,10))
ax = plt.axes(projection='3d')
ax.plot(numericResult[:,0],numericResult[:,1],numericResult[:,2])

ax.set_title("2 Body Problem",fontsize=10)
ax.set_xlabel('x [km]')
ax.set_ylabel('y [km]')
ax.set_zlabel('z [km]')
xlimits = [-2*10**4,1*10**4]
limits = [-1*10**4,1*10**4]
ax.set_xlim(xlimits)
ax.set_ylim(limits)
ax.set_zlim(limits)
