...
function in = inhull2(testpts,xyz,tess,tol)
...
% Specify the maximum no. of workers in parallel for loop:
m = 6;

parfor (j = 1:n, m)
%     for j = 1:n
%     progressbar(j/n,3);
   % Please check the logic of Jim's modification. It needs correction.
%    j = i:blocks:n;
%    if size(aNr,2) ~= length(j),
%       aNr = repmat(aN,1,length(j));
%    end
% j
   in(j) = all((nrmls*testpts(j,:)' - aN) >= -tol,1)';
end
% tocBytes(gcp)
% toc