...
CR3BPIC = returnCR3BPIC(orbitFamily,L=4,id=1172,stable=True)
...
def returnModel(modelString = 'mamba'):
    if modelString == 'mamba':
        ...
        model = LSTM(input_size,50,output_size,num_layers,0).double().to(device)
    return model

model = returnModel(modelToRun)
