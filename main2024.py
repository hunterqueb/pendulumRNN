...
for i in range(DATA_SET_SIZE):
    theta = [random.uniform(0.84, 0.86), 0]
...
for epoch in range(n_epochs):
    ...
    def closure():
        ...
        loss = criterion(out, trainingDataOutput)#,reduction='mean', beta=1e-6)
        print("     loss", loss.item())
        loss.backward()
        return loss
    optimizer.step(closure)
    # optimizer.zero_grad()
    # output = model(trainingDataInput)  # 'input' is a key in your dataset
    # loss = criterion(output, trainingDataOutput)  # 'target' is the target variable in your dataset
    # loss.backward()
    # optimizer.step()
